...
export default function Dashboard() {
  ...
  // Hover states for buttons and dropdowns
  ...
  const handleAddOrUpdate = async () => {
    if (!title || !description) return toast.warn("Please enter title & description");
    try {
      if (editingTaskId) {
  ...
  const handleShare = (task) => {
    ...
    if (navigator.share) {
      navigator.share({ title: task.title, text: task.description, url: taskLink });
    } else {
  ...
  const card = {
    ...
    padding: "20px",
    borderRadius: "12px",
    boxShadow: darkMode ? "0px 0px 15px rgba(0,0,0,0.5)" : "0px 0px 10px rgba(0,0,0,0.1)",
    display: "flex",
  ...
  const cardHover = { transform: "scale(1.02)", boxShadow: darkMode ? "0px 0px 25px rgba(0,0,0,0.7)" : "0px 0px 20px rgba(0,0,0,0.2)" };
  ...
  // Colorful dropdown style on hover
    ...
    <div style={container}>
      ...
      <div style={header}>
        ...
        <div style={profileArea}>
          <button onClick={toggleDarkMode} style={toggleBtn}>{darkMode ? "🌙" : "🌞"}</button>
      ...
      <div style={taskList}>
        ...
        {tasks.map(task => (
          ...
          >
            ...
            {task.updated_at && task.updated_at.getTime() !== task.created_at.getTime() && (
            ...
            <div style={btnGroup}>
              <button
                style={{ ...taskBtn, backgroundColor: "#f1c40f" }}
                onClick={() => handleEdit(task)}
                onMouseEnter={e => e.currentTarget.style.transform = "scale(1.05)"}
                onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
              >Edit</button>
              <button
                style={{ ...taskBtn, backgroundColor: "#e74c3c", color: "#fff" }}
                onClick={() => handleDelete(task.id)}
                onMouseEnter={e => e.currentTarget.style.transform = "scale(1.05)"}
                onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
              >Delete</button>
              <button
                style={{ ...taskBtn, backgroundColor: "#1abc9c", color: "#fff" }}
